...
- The protected cache is isolated by authenticated user, stores only readable responses, and retains at most 250 immutable article documents. See [Offline article reading](./offline-reading.md) for synchronization and mutation boundaries.
...
- If a public Substack page is throttled, acquisition checks the public post JSON endpoint, follows the publication feed to discover a migrated custom domain, checks full feed content, and only then may use Jina Reader's unauthenticated, rate-limited HTML rendering endpoint. Every result still passes Zine's normal extraction, sanitization, provenance, and quality gates.
...
- Dead-letter audit rows remain durable, but a later successful repair or authoritative terminal repair result marks matching item/version events resolved so queue health reflects the actionable backlog rather than permanent history.
