...
The native app uses ClerkKit and the `/api/v1` REST API. The web app uses
Clerk and tRPC. Both use the same backend connection, subscription, token,
and ingestion services described below.
...
See `apps/ios/ZineNative/Features/Subscriptions/ProviderOAuthSession.swift` and
`apps/ios/ZineNative/App/AppConfiguration.swift` for native provider configuration.
Local overrides belong in `apps/ios/Configuration/Local.xcconfig`.
...
`ProviderOAuthSession` generates PKCE and registers state through the REST API,
opens `ASWebAuthenticationSession`, validates the callback state, and sends the
code, verifier, redirect URI, and state to the backend for token exchange.
...
Native subscription views and stores live under
`apps/ios/ZineNative/Features/Subscriptions`. `APIClient` owns their REST calls.
Web subscription flows remain in `apps/web` and use the shared backend through tRPC.

## Complete End-to-End Flow

1. The client registers OAuth state, authorizes with PKCE, and submits the callback.
2. The Worker exchanges the code and stores encrypted provider credentials.
3. The user selects subscriptions; the Worker performs the initial fetch.
4. Scheduled polling and queued sync jobs ingest new canonical items.
5. The supported clients read the user's Inbox and Library through their API boundary.
...
- `apps/ios/ZineNative/Features/Subscriptions/ProviderOAuthSession.swift`: OAuth session and PKCE.
- `apps/ios/ZineNative/Core/API/APIClient.swift`: REST requests.
- `apps/ios/ZineNative/Features/Subscriptions`: subscription views and stores.
...
See `apps/ios/Configuration/Local.xcconfig.example` for supported local overrides.
Provider client secrets remain in the Worker.
