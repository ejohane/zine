...
# Function to determine the host web and physical-device clients should use.
...
if [ "$WORKTREE_PATH" = "$MAIN_WORKTREE" ]; then
  ...
  PREFERRED_WEB=$((8200 + PORT_OFFSET))
  WEB_PORT=$(find_available_port_from $PREFERRED_WEB)
  if [ "$WEB_PORT" != "$PREFERRED_WEB" ]; then
    echo "   ℹ️  Web port $PREFERRED_WEB in use, using $WEB_PORT instead"
  fi
fi
...
if [ "$DEV_HOST" != "localhost" ]; then
  ...
  echo "   Web:      http://$DEV_HOST:$WEB_PORT"
else
...
# --- Web Secrets ---
CURRENT_WEB_ENV_BACKUP=""
if [ -f apps/web/.env.local ]; then
    CURRENT_WEB_ENV_BACKUP=$(mktemp)
    cp apps/web/.env.local "$CURRENT_WEB_ENV_BACKUP"
fi

echo "   📝 Generating apps/web/.env.local ($API_BASE_URL)..."
mkdir -p apps/web
...
# Auto-generated by scripts/dev.sh for the web channel
